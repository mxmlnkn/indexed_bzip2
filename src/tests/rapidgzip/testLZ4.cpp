...
decodeLZ4Block( const std::vector<std::byte>& toDecompress )
{
    ...
    while ( i < toDecompress.size() ) {
        ...
        const auto matchStart = i - offset;
        ...
        //if ( matchLength > 100 ) {
        //    std::cerr << " Copy " << matchLength << " from offset " << -offset << " to offset: " << i << "\n";
        //}

        if ( matchLength <= offset ) {
            decompressed.insert( decompressed.end(), decompressed.begin() + matchStart,
                                 decompressed.begin() + matchStart + matchLength );
        } else {
            /** @todo */
            decompressed.resize( decompressed.size() + matchLength );
        }
    }
...
void
decodeLZ4( const std::string& filePath )
{
    const auto file = std::make_unique<StandardFileReader>( filePath );
    ...
    while ( true ) {
        ...
        if ( blockIsUncompressed ) {
            ...
            totalBytesDecoded += decodeLZ4Block( buffer ).size();
        }
