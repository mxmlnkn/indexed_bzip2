        ...
        with path.open('wb') as file:
            with gzip.GzipFile(fileobj=file, mode='wb', compresslevel=1) as gz:
                for _ in range(size_in_MiB):
                    gz.write(random.randbytes(1024*1024))
        return path
...
def test_issue_51(tmp_path):
    ...
    with gzip.open(path, 'wb') as file:
        file.write(gzip.compress(b"Hello World!"))

    with path.open('rb') as zfd:
        print("fileno:", zfd.fileno())
        unpacked_once = rapidgzip.open(zfd, parallelization=1)
        ...
        unpacked = rapidgzip.open(unpacked_once, parallelization=2)
