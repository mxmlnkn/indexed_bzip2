...
getUsedWindowSymbols( BitReader& bitReader )
{
    ...
    for ( size_t nBytesRead = 0; nBytesRead < MAX_WINDOW_SIZE; ) {
        ...
        for ( const auto& reference : backreferences ) {
            ...
            if ( reference.first < nBytesRead ) {
                continue;
            }

            const auto distanceFromEnd = reference.first - nBytesRead;
            ...
            const auto startOffset = distanceFromEnd == window.size() ? 0 : window.size() - 1U - distanceFromEnd;
            const auto length = distanceFromEnd == window.size() ? reference.second - 1U : reference.second;

            for ( size_t i = 0; ( i < length ) && ( startOffset + i < window.size() ); ++i ) {
                 ...
                 const Container& window )
{
    ...
    std::vector<uint8_t> sparseWindow( std::min( 32_Ki, window.size() ), 0 );
