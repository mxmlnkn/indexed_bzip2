      ...
      char** argv )
{
    ...
    /** @todo implement sparse window representation as contemplated for the IndexFileFormat! */
    size_t windowSizeSparseRepresentation{ 0 };
    ...
    std::cerr << "    Window Count: " << windowMap->size() << "\n"
              ...
              /** @todo */
              //<< "    Total Window Size Sparse Representation: "
              //<< formatBytes( windowSizeSparseRepresentation ) << "\n"
...
m benchmarkIndexCompression && src/benchmarks/benchmarkIndexCompression /media/e/wikidata-20220103-all.json.gz{,.index}
 ...
 -> Zlib has slightly higher (~15%) compression but takes much much longer!
...
stat --format=%s 4GiB-base64.gz.index
